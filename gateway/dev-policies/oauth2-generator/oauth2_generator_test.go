...
// the status code alone is enough to decide whether to self-retry, so this
// stays safe for streamed responses. RequestBodyMode switches to BUFFER so
// the original request body is available to replay against the backend on
// self-retry - see OnRequestBody.
func TestMode_PurgeEnabled_ProcessesResponseHeadersOnly(t *testing.T) {
	...
	if mode.RequestBodyMode != policy.BodyModeBuffer {
		t.Errorf("expected RequestBodyMode BUFFER when purgeStatusCodes is non-empty (needed to replay the body on self-retry), got %v", mode.RequestBodyMode)
	}
	...
	if mode.ResponseBodyMode != policy.BodyModeSkip {
		t.Errorf("expected ResponseBodyMode SKIP - the decision to self-retry only needs the status code; the failed response's body is simply discarded via the ImmediateResponse short-circuit, got %v", mode.ResponseBodyMode)
	}
}

// TestOnRequestBody_Passthrough locks in that OnRequestBody exists solely to
// trigger request-body buffering (see RequestPolicy's doc comment: any
// policy implementing it causes the kernel to buffer the body before the
// chain runs) - it must never mutate or block the request itself.
func TestOnRequestBody_Passthrough(t *testing.T) {
	p := newTestPolicy()
	reqCtx := &policy.RequestContext{
		SharedContext: &policy.SharedContext{},
		Body:          &policy.Body{Content: []byte(`{"hello":"world"}`), Present: true, EndOfStream: true},
	}
	action := p.OnRequestBody(context.Background(), reqCtx, nil)
	mods, ok := action.(policy.UpstreamRequestModifications)
	if !ok {
		t.Fatalf("expected UpstreamRequestModifications (passthrough), got %T", action)
	}
	if mods.Body != nil {
		t.Errorf("expected nil Body (passthrough, no mutation), got %q", mods.Body)
	}
}
...
// ─── OnResponseHeaders self-retry ───────────────────────────────────────────

// newSelfRetryResponseHeaderCtx builds a ResponseHeaderContext shaped like a
// real rejected request against backendURL: SharedContext.APIContext/
// APIVersion and RequestPath are set so reconstructBackendURL can resolve a
// target (see TestReconstructBackendURL for the formula).
func newSelfRetryResponseHeaderCtx(status int, backendURL string) *policy.ResponseHeaderContext {
	return &policy.ResponseHeaderContext{
		SharedContext:   &policy.SharedContext{APIContext: "/petstore", APIVersion: "v1.0.0"},
		RequestHeaders:  policy.NewHeaders(map[string][]string{"X-Original": {"yes"}}),
		RequestBody:     &policy.Body{Present: false},
		RequestPath:     "/petstore/v1.0.0/pets/123",
		RequestMethod:   http.MethodGet,
		ResponseHeaders: policy.NewHeaders(map[string][]string{}),
		ResponseStatus:  status,
		Upstream:        &policy.UpstreamResponseContext{URL: backendURL, BasePath: "/api/v2"},
	}
}

// TestOnResponseHeaders_SelfRetry_SuccessReplacesResponse proves the core
// self-retry flow end to end: purge, fetch a fresh token, replay the request
// against the reconstructed backend URL, and replace the original 401 with
// the retry's own response via ImmediateResponse.
func TestOnResponseHeaders_SelfRetry_SuccessReplacesResponse(t *testing.T) {
	...
	p.tokenSource = fake
	p.purgeStatusCodes = map[int]struct{}{http.StatusUnauthorized: {}}
	p.tokenFunc = func() (*xoauth2.Token, error) { return &xoauth2.Token{AccessToken: "fresh-token"}, nil }

	action := p.OnResponseHeaders(context.Background(), newSelfRetryResponseHeaderCtx(http.StatusUnauthorized, backend.URL), nil)

	imm, ok := action.(policy.ImmediateResponse)
	if !ok {
		t.Fatalf("expected ImmediateResponse (self-retry replaces the response), got %T", action)
	}
	if imm.StatusCode != http.StatusOK {
		t.Errorf("expected the retry's 200 status, got %d", imm.StatusCode)
	}
	if string(imm.Body) != `{"ok":true}` {
		t.Errorf("unexpected body: %q", imm.Body)
	}
	if imm.Headers["X-From-Backend"] != "yes" {
		t.Errorf("expected the backend's response header to be relayed, got %#v", imm.Headers)
	}
	if fake.purgeCalls != 1 {
		t.Errorf("expected exactly one Purge() call, got %d", fake.purgeCalls)
	}
	if gotAuth != "Bearer fresh-token" {
		t.Errorf("expected the retry to carry the fresh token, got %q", gotAuth)
	}
	if gotPath != "/api/v2/pets/123" {
		t.Errorf("expected the reconstructed backend path, got %q", gotPath)
	}
	if gotMethod != http.MethodGet {
		t.Errorf("expected the original method to be replayed, got %q", gotMethod)
	}
}

// TestOnResponseHeaders_SelfRetry_ReplaysRequestBody proves a non-GET
// request's body (available because Mode() now buffers it - see
// TestMode_PurgeEnabled_ProcessesResponseHeadersOnly) is replayed verbatim
// against the backend.
func TestOnResponseHeaders_SelfRetry_ReplaysRequestBody(t *testing.T) {
	var gotBody []byte
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	respCtx := newSelfRetryResponseHeaderCtx(http.StatusUnauthorized, backend.URL)
	respCtx.RequestMethod = http.MethodPost
	respCtx.RequestBody = &policy.Body{Content: []byte(`{"name":"fido"}`), Present: true, EndOfStream: true}
	...
	if imm.StatusCode != http.StatusCreated {
		t.Errorf("unexpected status: %d", imm.StatusCode)
	}
	if string(gotBody) != `{"name":"fido"}` {
		t.Errorf("expected the original request body to be replayed, got %q", gotBody)
	}
}

// TestOnResponseHeaders_SelfRetry_OverwritesStaleAuthorizationHeader proves
// the rejected token captured in RequestHeaders is replaced with the fresh
// one, not replayed as-is (which would just reproduce the same 401).
func TestOnResponseHeaders_SelfRetry_OverwritesStaleAuthorizationHeader(t *testing.T) {
	var gotAuth string
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAuth = r.Header.Get("Authorization")
		w.WriteHeader(http.StatusOK)
	...
	respCtx := newSelfRetryResponseHeaderCtx(http.StatusUnauthorized, backend.URL)
	respCtx.RequestHeaders = policy.NewHeaders(map[string][]string{"Authorization": {"Bearer stale-rejected-token"}})

	p.OnResponseHeaders(context.Background(), respCtx, nil)

	if gotAuth != "Bearer fresh-token" {
		t.Errorf("expected the stale Authorization header to be overwritten with the fresh token, got %q", gotAuth)
	}
}

// TestOnResponseHeaders_SelfRetry_NetworkFailureFailsOpen proves an
// unreachable backend on the retry attempt fails open - the token is still
// purged (so the *next* client request gets a fresh one), but the original
// response passes through unmodified rather than the retry ever being able
// to make a request fail in a new way.
func TestOnResponseHeaders_SelfRetry_NetworkFailureFailsOpen(t *testing.T) {
	fake := &fakeTokenSource{}
	p := newTestPolicy()
	p.tokenSource = fake
	p.purgeStatusCodes = map[int]struct{}{http.StatusUnauthorized: {}}
	...
	// Port 1 is reserved/unreachable - see TestPasswordGrant_EndToEnd's use of
	// the same pattern to pin Redis to a guaranteed-unreachable address.
	respCtx := newSelfRetryResponseHeaderCtx(http.StatusUnauthorized, "http://127.0.0.1:1")

	action := p.OnResponseHeaders(context.Background(), respCtx, nil)
	if _, ok := action.(policy.DownstreamResponseHeaderModifications); !ok {
		t.Fatalf("expected pass-through DownstreamResponseHeaderModifications on retry failure, got %T", action)
	}
	if fake.purgeCalls != 1 {
		t.Errorf("expected the token to still be purged even though the retry itself failed, got %d", fake.purgeCalls)
	}
}

// TestOnResponseHeaders_SelfRetry_TokenFetchFailureFailsOpen proves a failed
// fresh-token fetch fails open without ever calling the backend.
func TestOnResponseHeaders_SelfRetry_TokenFetchFailureFailsOpen(t *testing.T) {
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Fatal("backend should never be called when the fresh token fetch itself fails")
	}))
	defer backend.Close()
	...
	p.tokenSource = &fakeTokenSource{}
	p.purgeStatusCodes = map[int]struct{}{http.StatusUnauthorized: {}}
	p.tokenFunc = func() (*xoauth2.Token, error) { return nil, errors.New("token endpoint returned invalid_client") }

	action := p.OnResponseHeaders(context.Background(), newSelfRetryResponseHeaderCtx(http.StatusUnauthorized, backend.URL), nil)
	if _, ok := action.(policy.DownstreamResponseHeaderModifications); !ok {
		t.Fatalf("expected pass-through on token fetch failure, got %T", action)
	}
}
...
// TestBuildRetryRequest_ExcludesHTTP2PseudoHeaders proves HTTP/2 pseudo-headers
// (:path, :method, :authority, :scheme) - which the real ext_proc RequestHeaders
// snapshot does include as ordinary entries, confirmed live against a real
// gateway stack - are never cloned onto the outbound self-retry request.
// net/http's Transport rejects any header whose name starts with ":" outright
// ("invalid header field name"), which previously made every self-retry attempt
// fail closed the moment a real request (always carrying these) went through -
// a case the hand-written test fixtures elsewhere in this file never exercised
// because they only ever set ordinary header names.
func TestBuildRetryRequest_ExcludesHTTP2PseudoHeaders(t *testing.T) {
	respCtx := &policy.ResponseHeaderContext{
		RequestMethod: http.MethodGet,
		RequestHeaders: policy.NewHeaders(map[string][]string{
			":path":         {"/petstore/v1.0.0/pets/123"},
			":method":       {"GET"},
			":authority":    {"gateway.example.com"},
			":scheme":       {"https"},
			"X-Original":    {"yes"},
	...
	req, err := buildRetryRequest(context.Background(), "https://backend.example.com/api/v2/pets/123", respCtx, "Authorization", "Bearer fresh-token")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	for _, pseudo := range []string{":path", ":method", ":authority", ":scheme"} {
		if _, ok := req.Header[pseudo]; ok {
			t.Errorf("expected pseudo-header %q to be excluded, but it was set: %v", pseudo, req.Header[pseudo])
		}
	}
	if got := req.Header.Get("X-Original"); got != "yes" {
		t.Errorf("expected an ordinary header to still be cloned, got %q", got)
	}
	if got := req.Header.Get("Authorization"); got != "Bearer fresh-token" {
		t.Errorf("expected the fresh token to be set, got %q", got)
	}
}

// ─── reconstructBackendURL ────────────────────────────────────────────────

// TestReconstructBackendURL covers the best-effort path reconstruction
// self-retry relies on: Envoy's own context/version-stripping path rewrite
// (translator.go) runs AFTER ext_proc, so RequestPath is the pre-rewrite,
// client-facing path - not what was actually sent to the backend. This
// strips the API's own context(+version) prefix and prepends Upstream.BasePath,
// matching the common-case rewrite. Anything it isn't confident about (no
// prefix match, no Upstream, MCP's divergent resource-route rewrite) must
// return ok=false so the caller falls back to purge-only, never a guessed URL.
func TestReconstructBackendURL(t *testing.T) {
	tests := []struct {
		name    string
		respCtx *policy.ResponseHeaderContext
		wantURL string
		...
		{
			name: "context+version prefix match",
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/petstore", APIVersion: "v1.0.0"},
				RequestPath:   "/petstore/v1.0.0/pets/123",
				Upstream:      &policy.UpstreamResponseContext{URL: "https://backend.example.com", BasePath: "/api/v2"},
			},
			wantURL: "https://backend.example.com/api/v2/pets/123",
			wantOK:  true,
		},
		{
			name: "context-only prefix match (no version in path)",
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/weather", APIVersion: "v1.0.0"},
				RequestPath:   "/weather/us/seattle",
				Upstream:      &policy.UpstreamResponseContext{URL: "https://backend.example.com", BasePath: "/api/v2"},
			},
			wantURL: "https://backend.example.com/api/v2/us/seattle",
			wantOK:  true,
		},
		{
			name: "root path with empty basePath falls back to /",
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/petstore", APIVersion: "v1.0.0"},
				RequestPath:   "/petstore/v1.0.0",
				Upstream:      &policy.UpstreamResponseContext{URL: "https://backend.example.com", BasePath: ""},
			},
			wantURL: "https://backend.example.com/",
			wantOK:  true,
		},
		{
			name: "trailing slash on Upstream.URL and BasePath trimmed",
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/petstore", APIVersion: "v1.0.0"},
				RequestPath:   "/petstore/v1.0.0/pets",
				Upstream:      &policy.UpstreamResponseContext{URL: "https://backend.example.com/", BasePath: "/api/v2/"},
			},
			wantURL: "https://backend.example.com/api/v2/pets",
			wantOK:  true,
		},
		{
			...
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/petstore", APIVersion: "v1.0.0"},
				RequestPath:   "/something-unrelated/pets/123",
				Upstream:      &policy.UpstreamResponseContext{URL: "https://backend.example.com", BasePath: "/api/v2"},
			},
			wantOK: false,
		},
		...
		},
		{
			...
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/petstore", APIVersion: "v1.0.0"},
				RequestPath:   "/petstore/v1.0.0/pets/123",
				Upstream:      &policy.UpstreamResponseContext{URL: "", BasePath: "/api/v2"},
			},
			wantOK: false,
		},
		{
			// MCP's resource-route rewrite maps the whole gateway path to the
			// upstream URL regardless of suffix (see translator.go's
			// isMCPResourceRoute case) - a matching context prefix does NOT
			// guarantee our generic suffix-append formula is correct, so MCP
			// is excluded outright rather than risk a wrong URL.
			name: "MCP API kind excluded even with a matching prefix",
			respCtx: &policy.ResponseHeaderContext{
				SharedContext: &policy.SharedContext{APIContext: "/weather", APIVersion: "v1.0.0", APIKind: policy.APIKindMCP},
				RequestPath:   "/weather/v1.0.0/mcp",
				Upstream:      &policy.UpstreamResponseContext{URL: "https://backend.example.com", BasePath: "/mcp-server"},
			},
			wantOK: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, ok := reconstructBackendURL(tt.respCtx)
			if ok != tt.wantOK {
				t.Fatalf("ok = %v, want %v (url=%q)", ok, tt.wantOK, got)
			}
