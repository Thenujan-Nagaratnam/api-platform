...
type upstreamAttemptState struct {
	...
	// attempt (see extractMemberClusterName) — "" for a non-model-failover
	// route, or a route whose real cluster carries no chain-identity
	// metadata. Captured once at the RequestHeaders phase and reused for
	// every later phase of the same attempt (RouteCluster only ever reports
	// the aggregate's own name for an aggregate-routed attempt, so this is
	// the one field that actually distinguishes which real member is live).
...
func (s *UpstreamExternalProcessorServer) Process(stream extprocv3.ExternalProcessor_ProcessServer) error {
	...
	for {
		...
		case *extprocv3.ProcessingRequest_RequestHeaders:
			...
			state.memberClusterName = extractMemberClusterName(req)
			...
			if state.chain != nil && state.chain.RequiresUpstreamRequest {
				reqHdrCtx := BuildUpstreamAttemptRequestHeaderContext(state.sharedContext, state.rawClusterName, state.requestHeaders,
					state.clusterName, state.backendURL, state.basePath, state.method, state.path, state.memberClusterName)
		...
		case *extprocv3.ProcessingRequest_ResponseHeaders:
			...
			if state.chain != nil && state.chain.RequiresUpstreamResponse {
				respHdrCtx := BuildUpstreamAttemptResponseHeaderContext(state.sharedContext, state.rawClusterName, state.responseHeaders,
					state.clusterName, state.backendURL, state.basePath, state.method, state.path, state.statusCode, state.memberClusterName)
...
// memberClusterIdentityNamespace/Key match the filter_metadata entry
// gateway-controller stamps on every real cluster referenced by a
// model-failover chain (pkg/xds/member_cluster_identity.go's
// applyMemberClusterIdentityMetadata — the constants there must stay in
// lockstep with these).
const (
	memberClusterIdentityNamespace = "gateway.wso2.com/model-failover"
	memberClusterIdentityKey       = "cluster_name"
)

// extractMemberClusterName reads xds.upstream_host_metadata — the REAL
// member cluster Envoy actually dialed for this attempt, independent of
// xds.cluster_name (which reports only the aggregate's own name on every
// aggregate-routed attempt, confirmed live) and independent of
// x-envoy-attempt-count (not attempt-position-accurate once
// outlier_detection can make Envoy's load balancer skip a priority silently
// — see the model-failover design's chain-position-resolution discussion).
//
// Confirmed live: Envoy delivers this attribute as a STRING containing the
// textproto serialization of the dialed host's core.Metadata — not a
// structured CEL value — so it's parsed back with prototext rather than
// read as a plain string or a nested Struct. Returns "" if the host carries
// no such metadata (not a model-failover chain member) or the attribute is
// absent entirely.
func extractMemberClusterName(req *extprocv3.ProcessingRequest) string {
	if req.Attributes == nil {
		return ""
	}
	extProcAttrs, ok := req.Attributes[constants.ExtProcFilter]
	...
	v, ok := extProcAttrs.Fields["xds.upstream_host_metadata"]
	if !ok {
		return ""
	}
	s := v.GetStringValue()
	...
	var md corev3.Metadata
	if err := prototext.Unmarshal([]byte(s), &md); err != nil {
		return ""
	}
	return md.GetFilterMetadata()[memberClusterIdentityNamespace].GetFields()[memberClusterIdentityKey].GetStringValue()
}
