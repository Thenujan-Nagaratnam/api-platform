...
// It handles the request-headers phase (every attempt), additively the
// request-body phase for clusters whose ext_proc filter enables
// RequestBodyMode: BUFFERED (model-failover body rewriting, Task 3/6), and
// additively the response-headers phase for clusters whose ext_proc filter
// enables ResponseHeaderMode: SEND (read-only attempt observation via
// UpstreamAttemptResponseObserver, Task 4/9). It resolves route -> policy
// chain via the exact same
...
// Process implements extprocv3.ExternalProcessorServer. Three message types
// are possible now: RequestHeaders (every attempt), RequestBody (only for
...
// message at all), and ResponseHeaders (only for clusters whose ext_proc
// filter has ResponseHeaderMode: SEND — opt-in per cluster via
// gateway-controller's collectClustersNeedingUpstreamResponseObserver, Task
// 8 — sent once per attempt when the cluster carries a policy implementing
// UpstreamAttemptResponseObserver). attemptCount is parsed once from the
// request-headers message and reused for both the body message and the
// response-headers message on the same stream — all three arrive for the
// same individual dial attempt.
func (s *UpstreamExternalProcessorServer) Process(stream extprocv3.ExternalProcessor_ProcessServer) error {
	...
	for {
		...
		case *extprocv3.ProcessingRequest_RequestHeaders:
			...
			resp, attemptCount, err = s.processUpstreamAttemptRequestHeaders(ctx, req, routeKey)
			if err != nil {
		...
		case *extprocv3.ProcessingRequest_RequestBody:
			resp, err = s.processUpstreamAttemptRequestBody(ctx, v.RequestBody, routeKey, attemptCount)
			if err != nil {
		...
		case *extprocv3.ProcessingRequest_ResponseHeaders:
			resp, err = s.processUpstreamAttemptResponse(ctx, v.ResponseHeaders, routeKey, attemptCount)
			if err != nil {
...
// processUpstreamAttemptRequestHeaders resolves the route's policy chain and
// dispatches to every policy implementing UpstreamAttemptPolicy, in chain
// order. A policy that doesn't implement it (the common case — rate
// limiting, analytics, transforms) is silently skipped via the type
// assertion; this is what makes the mechanism generic with zero per-policy
// wiring in this server. routeKey is resolved once by Process and passed in
// so it can be reused for the body message on the same stream. It also
// returns the parsed attemptCount so Process can carry it forward to
// processUpstreamAttemptRequestBody.
...
// processUpstreamAttemptRequestBody dispatches the SAME OnUpstreamAttemptRequest
// hook again (not a new interface method — see the SDK design note in Task
// 2), this time with Body populated, and applies any returned Body mutation
// with a matching Content-Length. Policies that never set actx.Body-derived
// logic (headers-only consumers like oauth2-generator) simply won't have
// this method invoked at all for their clusters, since their clusters never
...
// emptyContinueResponseHeadersResponse is the fail-open / no-op response for
// the response-observation phase: no mutation is ever possible here
...
// Read-only: unlike the request-phase handlers, there is no action to apply
// back to Envoy — every policy just observes. attemptCount is the value
// already parsed from this same attempt's request-headers message (see
// Process()'s own attemptCount variable), reused here since Envoy does not
// repeat x-envoy-attempt-count on the response.
//
// respHeaders is *extprocv3.HttpHeaders, not a wrapping *HttpResponse type —
// Envoy's ProcessingRequest_ResponseHeaders oneof variant carries the same
// HttpHeaders payload type the request-headers case does (confirmed against
// the real generated external_processor.pb.go; there is no HttpResponse type
// in this proto package).
func (s *UpstreamExternalProcessorServer) processUpstreamAttemptResponse(ctx context.Context, respHeaders *extprocv3.HttpHeaders, routeKey string, attemptCount int) (*extprocv3.ProcessingResponse, error) {
	chain := s.kernel.GetPolicyChain(routeKey)
	if chain == nil {
		return emptyContinueResponseHeadersResponse(), nil
	}

	headers := respHeaders.GetHeaders()
	status := 0
	requestID := ""
	if headers != nil {
