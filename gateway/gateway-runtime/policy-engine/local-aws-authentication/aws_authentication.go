...
//
// UpstreamRequestMode additionally opts into the upstream-attempt phase (see
// OnUpstreamRequestBody): when this policy is bound to a specific backend in
// a model-failover fallback chain (e.g. an AWS Bedrock fallback for a
// non-AWS primary), signing must be re-resolved fresh on every attempt to
// that backend, not once for the whole client request — a stale signature
// computed for a different attempt would fail SigV4 validation outright.
...
// OnUpstreamRequestBody signs the outbound request with AWS SigV4, scoped to
// whichever backend this specific upstream attempt is targeting. Unlike
// OnRequestBody (invoked once per client request), the kernel invokes this
// fresh on every attempt — including a retry to a different AWS region or a
// different backend entirely — so the signature is always computed for the
// backend actually being dialed right now, never reused from a prior attempt.
func (p *AWSAuthenticationPolicy) OnUpstreamRequestBody(ctx context.Context, upCtx *policy.UpstreamAttemptContext, _ map[string]interface{}) policy.RequestAction {
	slog.Debug("AWSAuthentication: signing outbound request for attempt", "method", upCtx.Method, "path", upCtx.Path,
		"backend", upCtx.Name, "service", p.service, "region", p.region)

	var body []byte
	if upCtx.Body != nil {
		body = upCtx.Body.Content
	}

	creds, err := p.retrieveCredentials(ctx)
	if err != nil {
		return p.authFailure(upCtx.SharedContext, "failed to resolve AWS credentials", err)
	}

	signedHeaders, err := p.signRequestForAttempt(ctx, upCtx, body, creds)
	if err != nil {
		return p.authFailure(upCtx.SharedContext, "failed to sign AWS SigV4 request", err)
	}

	p.authSuccess(upCtx.SharedContext)
...
// signRequestForAttempt is signRequest's upstream-attempt counterpart: builds
// a synthetic *http.Request for THIS attempt's resolved backend and signs it.
func (p *AWSAuthenticationPolicy) signRequestForAttempt(ctx context.Context, upCtx *policy.UpstreamAttemptContext, body []byte, creds aws.Credentials) (map[string]string, error) {
	req, err := buildHTTPRequestForAttempt(upCtx, body)
	if err != nil {
...
// buildHTTPRequestForAttempt is buildHTTPRequest's upstream-attempt
// counterpart. Unlike RequestContext, UpstreamAttemptContext.Path is already
// the fully-resolved outbound path for this specific backend (BasePath
// already combined, no APIContext to strip), and URL is this attempt's
// backend — never the client-facing gateway host.
func buildHTTPRequestForAttempt(upCtx *policy.UpstreamAttemptContext, body []byte) (*http.Request, error) {
	if upCtx.URL == "" {
		return nil, fmt.Errorf("no resolved upstream URL for this attempt")
	}

	upstreamURL, err := url.Parse(upCtx.URL)
	if err != nil {
		return nil, fmt.Errorf("invalid upstream URL %q: %w", upCtx.URL, err)
	}

	rawURL := strings.TrimSuffix(upCtx.URL, "/") + upCtx.Path
	req, err := http.NewRequest(upCtx.Method, rawURL, bytes.NewReader(body))
	if err != nil {
		return nil, err
	}
	...
	if upCtx.Headers != nil {
		if ct := upCtx.Headers.Get("content-type"); len(ct) > 0 {
