...
func (s *LLMDeploymentService) DeployLLMProviderConfiguration(params LLMDeploymentParams) (*APIDeploymentResult, error) {
	...
	if err := config.ValidateModelFailoverForOperations(&apiConfig.Spec); err != nil {
...
func (s *LLMDeploymentService) DeployLLMProxyConfiguration(params LLMDeploymentParams) (*APIDeploymentResult, error) {
	...
	// Model-failover registration-time validation (target/fallback upstreamDefinition
	// references exist, no resilience.retry conflict, no basePath-carrying upstream used as
	// an aggregate-cluster member). Runs on apiConfig.Spec — already the fully-resolved
