	...
	return api.OperationPolicy{
		...
		Paths: []api.OperationPolicyPath{{
			...
			Params: map[string]interface{}{
				// Flat shape: a target entry's own model/provider sit directly
				// on the entry, at the same level fallbacks[] entries do — no
				// nested "target" wrapper (design §3's flattened params shape).
				"targets": []interface{}{
...
// TestTransform_ModelFailoverPolicy_ExplicitProviderOnPrimaryTargetIsReferenced
// guards the flat targets[] shape (design §3's "Post-implementation revision"):
// a chain's own primary slot can explicitly name a provider other than the
// proxy's default (authored exactly like a fallback slot), and that provider
// must be detected as failover-referenced from the TARGET alone — not only
// from a fallback naming it. A regression here (decoding the target's
// model/provider as nested under a "target" key, the pre-flattening shape)
// would silently skip this provider, leaving its credential attached both
// downstream (firing unconditionally) and upstream (leaking a spurious
// credential on a cross-provider retry, exactly what §8 exists to prevent).
func TestTransform_ModelFailoverPolicy_ExplicitProviderOnPrimaryTargetIsReferenced(t *testing.T) {
	db, transformer := newFailoverAuthTestStore(t)
	saveFailoverAuthTestProvider(t, db, "mistral-provider", "/mistral-provider")
	...
	proxy.Spec.AdditionalProviders = &additional
	// The chain's own primary slot explicitly names mistral-upstream — a
	// provider distinct from both the proxy's own default primary and the
	// existing fallback (anthropic-upstream) — so detection can only succeed
	// by reading the target's own model/provider, never by incidentally
	// matching the fallback's.
	...
	// The proxy's own default provider (openai-provider) is NOT referenced by
	// this chain at all — its own target explicitly points at mistral-upstream
	// instead — so openai-provider's unrelated downstream credential correctly
	// remains; only mistral-upstream's own downstream attachment must be gone.
	for _, p := range credentialAttachments(downstreamAttachments(policies, constants.UPSTREAM_AUTH_APIKEY_POLICY_NAME)) {
		if p.ExecutionCondition != nil && strings.Contains(*p.ExecutionCondition, "mistral-upstream") {
			t.Fatalf("mistral-upstream is the chain's own primary target and must not keep its downstream credential")
		}
	}
	assert.Empty(t, downstreamAttachments(policies, "openai-to-mistral-transformer"),
		"mistral-upstream's transformer must not keep its downstream attachment either")

	upstreamMistralAuth := credentialAttachments(upstreamAttachments(policies, constants.UPSTREAM_AUTH_APIKEY_POLICY_NAME))
	require.Len(t, upstreamMistralAuth, 1, "mistral-upstream must gain an upstream-attempt credential instance")
	require.NotNil(t, upstreamMistralAuth[0].ExecutionCondition)
	assert.Contains(t, *upstreamMistralAuth[0].ExecutionCondition, "mistral-upstream")

	upstreamMistralTransformer := upstreamAttachments(policies, "openai-to-mistral-transformer")
	require.Len(t, upstreamMistralTransformer, 1, "mistral-upstream must gain an upstream-attempt transformer instance")
}
