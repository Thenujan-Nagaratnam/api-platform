...
func (v *MCPValidator) validateUpstream(fieldPrefix string, upstream *api.MCPProxyConfigData_Upstream, definitions *[]api.UpstreamDefinition) []ValidationError {
	...
	// validateUpstreamAuthFields in llm_validator.go - since MCPProxyConfigData's
	// upstream.auth is generated from the same OpenAPI UpstreamAuth schema and
	// supports the same api-key/oauth2 shapes.
	if upstream.Auth != nil {
		...
		errors = append(errors, validateUpstreamAuthFields(fmt.Sprintf("%s.auth", fieldPrefix), fields)...)
	}
