...
func failoverRDC() *models.RuntimeDeployConfig {
	return &models.RuntimeDeployConfig{
		Metadata: models.Metadata{
			UUID: "api-3", Kind: "LlmProxy", Handle: "chat",
			Version: "v1", DisplayName: "Chat", ProjectID: "proj-1",
		},
		Context:             "/chat",
		PolicyChainResolver: models.RouteKeyResolverName,
		Routes: map[string]*models.Route{
			"POST|/chat/completions|main": {
				Method: "POST", Path: "/chat/completions", OperationPath: "/completions",
				Vhost: "main",
		...
		PolicyChains: map[string]*models.PolicyChain{
			"POST|/chat/completions|main": {Policies: []models.Policy{}},
		},
	}
}

// The chain[0]-is-the-target, chain[1:]-are-fallbacks-in-order shape is the
// contract `x-envoy-attempt-count - 1` indexes into per the design spec §6, and
// aggregate_cluster must match xds.AggregateClusterName exactly, since that is
// the same name Envoy reports back via xds.cluster_name.
func TestRouteConfigEmitsFailoverTargets(t *testing.T) {
	const routeKey = "POST|/chat/completions|main"

	resources, err := testTranslator().TranslateRuntimeConfigs([]*models.RuntimeDeployConfig{failoverRDC()})
	require.NoError(t, err)
	...
	assert.Equal(t, xds.AggregateClusterName(routeKey, 0), entry["aggregate_cluster"])
...
func TestRouteConfigOmitsFailoverTargetsWhenUnset(t *testing.T) {
	resources, err := testTranslator().TranslateRuntimeConfigs([]*models.RuntimeDeployConfig{restRDC()})
	require.NoError(t, err)
