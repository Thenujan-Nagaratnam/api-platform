...
type modelFailoverParams struct {
	...
	// SuspendAfterFailures is the number of CONSECUTIVE qualifying failures
	// required before a target is suspended. Passed straight through to the
	// policy's own params — suspension is policy-side state
	// (gateway/dev-policies/model-failover's suspensionState), never Envoy-native
	// outlier_detection: applying both independently would let Envoy's own LB
